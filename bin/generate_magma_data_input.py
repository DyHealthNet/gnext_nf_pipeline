...
def load_bim_ids(bim_path: str, genome_build: str) -> set:
    ...
    names=["Chromosome", "ID", "CM", "Position", "Allele1", "Allele2"]
)
...
def generate_MAGMA_gwas_input_file(gwas_path: str, phenocode: str, bim_ids: set, genome_build: str) -> pd.DataFrame:
...
def main():
    ...
    # Load data
    bim_ids = load_bim_ids(args.ref_bim, args.genome_build)
    
    input_files_dt = pd.read_csv(args.input_files, sep="\t", header=None, names=["file_path", "phenocode"])
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        futures = [
            executor.submit(generate_MAGMA_gwas_input_file, row['file_path'], row['phenocode'], bim_ids, args.genome_build)
            for idx, row in input_files_dt.iterrows()
        ]
        for future in as_completed(futures):
            try:
                magma_file = future.result()
                print(f"[OK] Generated {magma_file}", file=sys.stderr)
