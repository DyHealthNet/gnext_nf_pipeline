...
include { generate_magma_data_input } from './modules/local/magma.nf'
...
workflow {
    ...
    gwas_tuples = gwas_rows.map { row -> tuple(row.filename, row.phenocode) }

    // 2. Normalize per trait in batches (of size params.pheno_batch_size)
    gwas_batches = gwas_tuples.collate(params.pheno_batch_size) 
